  ...
  <Card class="p-card p-component">
    <template #title>
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2"> 
          <span>Баланс тимлидера</span>
        </div>
        <Tag severity="info" value="TEAMLEADER" />
      </div>
    </template>
    <template #content>
      <div class="flex mb-3 flex-wrap gap-2 items-end">
        ...
        <div class="flex items-center gap-2">
          ...
          <span class="font-medium text-gray-900 dark:text-gray-200">{{ walletStats.lockedForWithdrawalBalances.teamleader.primary }} {{ primaryCurrency }}</span>
        </div>
        <div class="flex gap-2 ml-auto">
          <Button
            ...
            severity="secondary"
            size="small"
            @click.prevent="openGlobalWithdrawalModal('teamleader', {user: user})"
          />
          <Button
            v-if="viewStore.isAdminViewMode"
            icon="pi pi-plus"
            label="Пополнить"
            size="small"
            @click.prevent="modalStore.openDepositModal({user}); setBalanceTypeForDeposit('teamleader')"
          />
          <Button
    ...
    <template #footer>
      <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">
        Баланс для операций тимлидера (приём и вывод средств)
      </div>
    </template>
  </Card>
</template>
