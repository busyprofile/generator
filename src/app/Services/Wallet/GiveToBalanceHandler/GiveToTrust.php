...
class GiveToTrust extends GiveToBalance
{
    public function handle(Wallet $wallet, Money $amount, TransactionType $transactionType): void
    {
        ...
        // Используем индивидуальный целевой резерв или глобальный по умолчанию
        $targetAmount = $wallet->target_reserve_amount ?? Wallet::RESERVE_BALANCE;
        $reserveNeeded = Money::fromPrecision($targetAmount, $wallet->reserve_balance->getCurrency());
