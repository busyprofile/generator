...
class Parser
{
    public function parse(string $sender, string $message): ?ParserResultValue
    {
        ...
        // Определяем валюту из SMS сообщения
        $currency = $this->detectCurrencyFromMessage($message, $gateway);

        $card = $this->parseCardLastDigitsFromMessage($message);

        return new ParserResultValue(
            amount: Money::fromPrecision($amount, $currency),
    ...
    {
        // Возвращаем старый рабочий regex
        $amountRegex = '(\s|\+)(?<amount>\d+(.\d+){0,3})\s{0,1}(RUB|rub|р|p|₽|RUR|rur|rurcard2card|руб|₸|kzt|TJS|tjs|сом|сомони)(\s|\.|\,|\;|$)';
    ...
    /**
     * Определяет валюту из SMS сообщения
     */
    private function detectCurrencyFromMessage(string $message, PaymentGateway $gateway): Currency
    {
        $message = NormalizeMessage::normalize($message);
        
        // Карта валютных обозначений с более точными паттернами
        $currencyPatterns = [
            'tjs' => [
                '/\btjs\b/i',
                '/\bсом\b/i', 
                '/\bсомони\b/i',
                '/\d+\s*сом\b/i',  // цифра + сом
            ],
            'rub' => [
                '/\brub\b/i',
                '/\bруб\b/i',
                ...
                '/rurcard2card/i',
                '/\d+\s*р\b/i',   // цифра + р
                '/\d+р\b/i',      // цифра+р слитно (как в SMS)
                '/\bрублей\b/i',
                '/\bрубля\b/i',
            ],
            'kzt' => [
        ...
        // Ищем совпадения в тексте SMS с regex паттернами
        foreach ($currencyPatterns as $currencyCode => $patterns) {
            foreach ($patterns as $pattern) {
                if (preg_match($pattern, $message)) {
                    try {
                        return Currency::make($currencyCode);
                    } catch (\Exception $e) {
                        // Валюта не поддерживается, продолжаем поиск
                        continue;
                    }
                }
            }
        ...
        // Если валюта не определена из SMS, используем валюту платежного шлюза
