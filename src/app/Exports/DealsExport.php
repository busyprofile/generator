...
class DealsExport implements FromCollection, WithHeadings, WithMapping, WithCustomStartCell, WithColumnFormatting
{
    ...
    public function map($row): array
    {
        ...
        return [
            ...
            (string) $order->paymentGateway?->code ?? 'partner',
            (string) $order->paymentGateway?->name ?? 'Партнерский шлюз',
            "'".$order->paymentDetail?->detail ?? 'Партнерские реквизиты',
            (string) $order->paymentDetail?->name ?? 'Партнер',
